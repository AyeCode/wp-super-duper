...
- **Never carry a group override when converting `add_background_group()` with no prefix.** Old code such as `sd_get_background_inputs( 'bg', [ 'group' => 'background' ] )` may specify a group, but when there is no prefix the conversion is always plain `->add_background_group()`. Only pass a group override when the call uses a non-empty prefix (e.g. `->add_background_group( 'card_' )`) and those fields genuinely belong in a different group than the default.
