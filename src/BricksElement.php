...
/**
 * WP Super Duper Bricks Builder Element
 *
 * Integrates Super Duper widgets as native Bricks Builder elements.
 *
 * @version 3.0.4-beta
 */
...
class BricksElement extends \Bricks\Element {
	...
	public function get_label() {
		...
		return str_replace( ' &gt; ', ' > ', $escaped_text );
	}
	...
	public function set_controls() {
		$args = $this->sd_convert_arguments();
	...
	/**
	 * Set the bricks control groups from the SD ones.
	 */
	...
	/**
	 * Values can never be arrays so convert if bricks setting makes it an array.
	 *
	...
	public function sd_convert_arguments() {
		...
		if ( ! empty( $args ) ) {
			foreach ( $args as $key => $arg ) {
				...
				// Bricks doesn't render dropdown when first option key is 0.
	...
	/**
	 * Convert the SD element_require to the Bricks required syntax.
	 *
	 * @param string $element_require
	 * @return array
	 */
	public function sd_convert_required( $element_require ) {
		...
		if ( strpos( $element_require, '||' ) !== false ) {
			...
			if ( $matches ) {
				...
				$bricks_required[]   = array( $control_id, '=', $values );
			}
		...
		foreach ( $matches as $match ) {
			...
			$bricks_required[] = array( $control_id, $operator, $value );
		}
...
/**
 * Implements desktop, tablet and mobile breakpoint views for fields hidden on
 * specific device types, adding device icons after field labels.
 */
add_action(
	'wp_enqueue_scripts',
	function () {
...
(function () {
    function getCurrentBreakpoint() {
        ...
        if (bricksPreview.classList.contains('desktop')) return 'desktop';
        else if (bricksPreview.classList.contains('tablet_portrait')) return 'tablet';
        else if (bricksPreview.classList.contains('mobile_landscape') || bricksPreview.classList.contains('mobile_portrait')) return 'phone';
        return null;
    }
    ...
    function groupFields() {
        ...
        controls.forEach((control) => {
            ...
            const baseKey = controlKey.replace(/(_sm|_md|_lg)$/, '');
            if (!grouped[baseKey]) grouped[baseKey] = { base: null, md: null, lg: null };
            if (controlKey.endsWith('_lg')) grouped[baseKey].lg = control;
            else if (controlKey.endsWith('_md')) grouped[baseKey].md = control;
            else grouped[baseKey].base = control;
        });
        return grouped;
    }
    ...
    function updateControlVisibility() {
        ...
        Object.keys(groupedFields).forEach((baseKey) => {
            ...
            if (!md && !lg) { if (base) base.style.display = ''; return; }
    ...
    const panelElementObserver = new MutationObserver(() => { updateControlVisibility(); });
    const bricksPanelElement = document.getElementById('bricks-panel-element');
    if (bricksPanelElement) panelElementObserver.observe(bricksPanelElement, { childList: true, subtree: true });

    const bricksPreviewObserver = new MutationObserver(() => { updateControlVisibility(); });
    const bricksPreview = document.getElementById('bricks-preview');
    if (bricksPreview) bricksPreviewObserver.observe(bricksPreview, { attributes: true, attributeFilter: ['class'] });
...
(function () {
    ...
    const panelElementObserver = new MutationObserver(() => { addIconsToLabels(); });
    const bricksPanelElement = document.getElementById('bricks-panel-element');
    if (bricksPanelElement) panelElementObserver.observe(bricksPanelElement, { childList: true, subtree: true });
