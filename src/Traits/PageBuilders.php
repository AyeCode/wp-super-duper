...
trait PageBuilders {
	 ...
	 *
	 * Only called when \Bricks\Elements exists (guarded in initialize_super_duper()).
	 * Triggers the SPL autoloader for BricksElement (which extends \Bricks\Element)
	 * and registers the backward-compat alias for Super_Duper_Bricks_Element.
	 */
	public function load_bricks_element_class() {
		// \Bricks\Element is guaranteed to exist at this point (guarded by the caller).
		// Trigger autoloading and set up the backward-compat alias in one step.
		if ( ! class_exists( 'Super_Duper_Bricks_Element', false ) ) {
			class_alias( 'AyeCode\\SuperDuper\\BricksElement', 'Super_Duper_Bricks_Element' );
		}
	}
	 ...
	 * This now acts as a wrapper for the global \AyeCode\SuperDuper\Utils::is_preview() helper function.
	 *
	 * @return bool
	 */
