...
trait ShortcodeInserter {
	...
	public static function get_picker( $editor_id = '' ) {
		...
		if ( isset( $_POST['editor_id'] ) ) {
			$editor_id = esc_attr( wp_unslash( $_POST['editor_id'] ) );
		...
		<div class="sd-shortcode-right-wrap">
			...
			<div id='sd-shortcode-output-actions'>
				...
				<?php endif; ?>
				<button class="button" onclick="sd_copy_to_clipboard()"><?php _e( 'Copy shortcode', 'ayecode-connect' ); ?></button>
			</div>
		</div>
		<?php
		$html = ob_get_clean();
	...
	public static function get_widget_settings() {
		...
		$shortcode = isset( $_REQUEST['shortcode'] ) ? sanitize_title_with_dashes( wp_unslash( $_REQUEST['shortcode'] ) ) : '';
		...
		$html = "<form id='" . esc_attr( $shortcode ) . "'>" . $form . "<div class=\"widget-control-save\"></div></form>";

		wp_send_json_success(
			array(
				'html' => $html,
				'css'  => $widget->widget_css(),
				'js'   => $widget->widget_js(),
			)
		);
	}
                ...
                jQuery.post(ajaxurl, data, function(response) {
                    ...
                    if ( response && response.success && response.data ) {
                        ...
                        if ( response.data.css ) {
                            jQuery('#sd-widget-settings-css').remove();
                            jQuery('<style id="sd-widget-settings-css">').text(response.data.css).appendTo('head');
                        }

                        if ( response.data.js ) {
                            jQuery('#sd-widget-settings-js').remove();
                            const script = document.createElement('script');
                            script.id = 'sd-widget-settings-js';
                            script.textContent = response.data.js;
